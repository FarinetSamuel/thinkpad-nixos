# Service de sauvegarde de la configuration NixOS

{ config, pkgs, lib, ... }:

{
  # Créer le répertoire pour les scripts
  system.activationScripts.mkBackupScriptDir = ''
  ...
  environment.etc."nixos/scripts/backup-nixos.sh" = {
    text = ''
      ...
      # Configuration
      BACKUP_DIR="/home/moi/nixos-backups"
  ...
  # Service systemd pour la sauvegarde automatique
  systemd.services.nixos-config-backup = {
    description = "Sauvegarde de la configuration NixOS";
    startAt = "weekly"; # Peut être défini à daily, weekly, etc.
    serviceConfig = {
      Type = "oneshot";
